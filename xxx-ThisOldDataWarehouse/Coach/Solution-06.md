...
1. Create a new alert rule with the following settings 
   - Scope – Synapse Workspace 
   - Condition - Whenever the Total of Custom log search is greater than 5
      - Search Query
   ...
   - Split by dimensions – ServerPrincipalName 
   - Operator – Greater than
   - Threshold Value – 5
   - Frequency of evaluation – 5 minutes
1. Create Action Group
   - Select appropriate Subscription, Resource Group and Region 
   - Supply a unique name for Action Group 
   - Supply a Display name for Action Group 
   - Create a notification type “Email/SMS message/Push/Voice” with unique name and select both Email and SMS boxes.  Fill out desired email and phone number
1. Details 
   - Severity - Critical 
   - Alert rule name – unique name  
1. Review and Create 
...
1. Under “Security incidents and alerts” click on the hyper link “Check for alerts on this resource in Microsoft Defender for Cloud”
   - Select “Sample Alerts” from the top 
   - Select appropriate subscription 
   - Select only “Azure SQL Databases” for Defender for Cloud plans 
   - Create sample alerts
   - Remove the filter for “Affected resource contains” and you should see the sample alerts
   - Review and explore alerts 
   - Change the status of each alert to “Resolved” 
...
1. Under “Findings” resolve all High severity entries 
   - Add Principal “sqlauditadmin” to baseline and rerun scan to confirm resolution
...
1. Column Level Security 
   - Path to sql script
1. Data Masking 
   - Using Azure Portal navigate to “Dynamic Data Masking” under Security pane within the Synapse Workspace SQL Pool
   - Click “Add mask”
      - Schema – Fact 
      - Table – Sale 
      - Column – Tax Amount, Tax Rate, Total Excluding Tax and Total Including Tax 
      - Masking field format – Default 
      - Click Add and repeat for each of the columns 
   - Verify users cannot see valid data for Tax related columns 

**Managed Identity** 

1. Using Azure Portal navigate to “Managed Identities” under Settings pane within the Azure Data Factory resource.
   - Make note of Object ID and name of the ADF resource 
...
1. Configure Source to connect Azure Synapse SQL Pool table Fact.Sale 
   - Create new Synapse Analytics Dataset for Fact.Sale
   - Create Linked Service leveraging Authentication type “System Assigned Managed Identity”
      - Ensure test connection is successful as expected 
   - Verify Dataset is connected successfully by leveraging the “Preview data” option 
1. Configure Sink to connect to Azure Synapse SQL Pool and create a new table called Fact.SaleCopy 	
   - Create new Synapse Analytics Dataset for Fact.SaleCopy
   - Use the same Linked Service created for the Source configuration 
   - Configure Sink Options
      - Set “Table option” to “Auto create table”
      - Set “Copy method” to “Bulk insert"
1. Execute ADF Pipeline
   - Verify creation of Fact.SaleCopy and the corresponding data is available 
