...
- Create power automate (flow) with name Get_SAP_Material (note: Name can be anything). <div class="color-text-danger mb-2">.color-text-danger<b>Note that this step requires input from power application which is not yet ready.</b></div> Create the flow and come back later to complete. Insert 3 steps in PowerApps as similar to earlier but with different SAP BAPI. Flow may look like below:
