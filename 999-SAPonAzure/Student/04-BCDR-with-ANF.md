...
2. Backup using a permanent solution (ANF snapshots):
	...
	- **Adjust** log backup volume size for storing log backups based on the size requirement (daily change of 250 GB) from Azure NetApp Files blade in Azure Portal, and **adjust** relevant HANA parameters (basepath_catalogbackup, basepath_logbackup) to use this volume for log backups. In addition, change the hana log backup timeout value in seconds, "log_backup_timeout_s", to align with the backup requirement - use HANA Studio. You may also want to confirm that the new log backup location has correct <sid>adm user permissions (chown -R user:group <new_backup_location>).
