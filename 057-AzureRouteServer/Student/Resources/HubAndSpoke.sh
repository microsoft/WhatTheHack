...
rg=your_resource_group  #STUDENT SHOULD REPLACE RGNAME WITH THEIR INITIALS
...
# Verify that student has changed rg and password variables
# If not, ask for them
...
az vm create -n hubvm -g $rg -l $location --image ubuntuLTS  \
    ...
    --public-ip-address hubvm-pip --public-ip-sku Standard \
    --vnet-name $vnet_name --size Standard_B1s --subnet $hub_vm_subnet_name \
...
az vm create -n spoke1-vm -g $rg -l $location --image ubuntuLTS  \
        --admin-username "$username" \
        --admin-password "$adminpassword" \
        --public-ip-address spoke1-vm-pip --public-ip-sku Standard\
        --vnet-name $spoke1_name --size Standard_B1s --subnet $spoke1_vm_subnet_name \
        -o none --only-show-errors
...
az vm create -n spoke2-vm -g $rg -l $location --image ubuntuLTS  \
        --admin-username "$username" \
        --admin-password "$adminpassword" \
        --public-ip-address spoke2-vm-pip --public-ip-sku Standard \
        --vnet-name $spoke2_name --size Standard_B1s --subnet $spoke2_vm_subnet_name \
        -o none --only-show-errors
