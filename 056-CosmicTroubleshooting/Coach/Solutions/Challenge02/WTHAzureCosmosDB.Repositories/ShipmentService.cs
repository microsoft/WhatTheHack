...
public class ShipmentService : ICosmosDbService<Shipment>
{
    ...
    {
        var response = await this._container.DeleteItemAsync<Shipment>(id, new PartitionKey(partitionKey));
        Console.WriteLine($"Shipment.DeleteItemAsync(id, partKey) Request Charge: {response.RequestCharge} RU/s");
    }
